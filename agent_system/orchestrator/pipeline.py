...
from agent_system.core.exceptions import OrchestratorError
...
class AgentPipeline:
    ...
    async def run(self, raw_query: str) -> FusedResult:
        ...
        api_result, web_result = await asyncio.gather(
            self._safe_execute(self._api_agent, context),
            self._safe_execute(self._web_agent, context),
        )
